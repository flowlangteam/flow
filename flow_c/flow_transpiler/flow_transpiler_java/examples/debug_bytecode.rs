...
fn main() {
    ...
    println!("Constant pool count: {} (means {} entries)", cp_count, cp_count - 1);
    pos += 2;
    ...
    for i in 1..cp_count {
        ...
        match tag {
            ...
            3 => { // Integer
                let val = i32::from_be_bytes([bytecode[pos], bytecode[pos+1], bytecode[pos+2], bytecode[pos+3]]);
                println!("Integer: {}", val);
                pos += 4;
            }
            5 => { // Long
            ...
            10 => { // Methodref
                ...
                println!("Methodref: class_index=#{}, name_and_type_index=#{}", class_idx, nat_idx);
                pos += 4;
            }
            12 => { // NameAndType
                let name_idx = u16::from_be_bytes([bytecode[pos], bytecode[pos+1]]);
                let desc_idx = u16::from_be_bytes([bytecode[pos+2], bytecode[pos+3]]);
                println!("NameAndType: name_index=#{}, descriptor_index=#{}", name_idx, desc_idx);
                pos += 4;
            }
            _ => {
    ...
    println!("Next bytes: {:02x} {:02x} {:02x} {:02x}", 
             bytecode[pos], bytecode[pos+1], bytecode[pos+2], bytecode[pos+3]);
}
